...
#prepare data
preprocessed_df_rf = sample_dataframe(prepare_dataframe_full(simplify_dataframe_full(df_airlines_weather)[0])[0])
preprocessed_df_rf.cache()
...
grid = ParamGridBuilder()\
        ...
        .addGrid(rf.numTrees, [20, 22, 25])\
        .addGrid(rf.minInstancesPerNode, [35, 40, 45])\
        .build()

feature_count, ranked_features = determine_optimal_feature_count(rf, preprocessed_df_rf, feature_counts, evaluator)
model_rf, scaler = fit_optimal_model(rf, preprocessed_df_rf, feature_count, ranked_features, evaluator, grid)
display(ranked_features)

# COMMAND ----------

#test model
preprocessed_df_19_rf = prepare_dataframe_full(simplify_dataframe_full(df_airlines_weather_19)[0])[0]
preprocessed_df_19_rf.cache()
predictions_rf = test_model(model_rf, preprocessed_df_19_rf, evaluator, feature_count=feature_count, ranked_features=ranked_features)
...
preprocessed_df_lr = sample_dataframe(prepare_dataframe_full(df)[0])
preprocessed_df_lr.cache()
...
grid = ParamGridBuilder()\
        ...
        .addGrid(lr.aggregationDepth, [4, 8, 12])\
        .build()

model_lr, scaler = fit_optimal_model(lr, preprocessed_df_lr, None, None, evaluator, grid, standardize=True, scaled_features_col="scaled_features", reduce_features=False)

# COMMAND ----------

#Test model
preprocessed_df_19_lr = prepare_dataframe_full(simplify_dataframe_full(df_airlines_weather_19, ohe=True, encoder=encoder)[0])[0]
preprocessed_df_19_lr.cache()
predictions_lr = test_model(model_lr, preprocessed_df_19_lr, evaluator, scaler=scaler, reduce_features=False)
...
df, encoder = simplify_dataframe_full(df_airlines_weather, ohe=True)
preprocessed_df_lrw, wt = prepare_dataframe_full(df, include_wt=True)
...
model_lrw, scaler = fit_optimal_model(lr, preprocessed_df_lrw, None, None, evaluator, grid, standardize=True, scaled_features_col="scaled_features", reduce_features=False)
...
model_rf.write().save(f"{blob_url}/models/rf_optimized")

# COMMAND ----------

model_lr.write().save(f"{blob_url}/models/lr_optimized")
...
predictions_rf.write.mode("overwrite").parquet(f"{blob_url}/predictions/rf_optimized")

# COMMAND ----------

predictions_lr.write.mode("overwrite").parquet(f"{blob_url}/predictions/lr_optimized")
